...
const { sendEmail, otpTemplate } = require('../config/mailer');
...
exports.sendVerifyOtp = async (req, res) => {
  try {
    ...
    try {
      await sendEmail(user.email, '🔐 Verify Your Email - GhummGhamm',
...
exports.sendResetOtp = async (req, res) => {
  try {
    ...
    try {
      await sendEmail(email, '🔐 Reset Password - GhummGhamm',
        otpTemplate(otp, user.name)
      );
    } catch (error) {
      return res.status(500).json({ success: false, message: 'Failed to send OTP: ' + error.message });
